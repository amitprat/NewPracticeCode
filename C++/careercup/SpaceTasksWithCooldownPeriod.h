...
public:
    static void test() {
        ...
        {
            vector<char> tasks = { 'A', 'B', 'C', 'D' }; int k = 3;
            string result;
        ...
        {
            vector<char> tasks = { 'A', 'B', 'A', 'D' }; int k = 3;
            string result;
        ...
        {
            vector<char> tasks = { 'A', 'A', 'A', 'A' }; int k = 3;
            string result;
        ...
        {
            vector<char> tasks = { 'A', 'B', 'C', 'A', 'C', 'B', 'D','A' }; int k = 4;
            string result;
    ...
    int spaceTasks(vector<char> tasks, int k, string& result) {
        ...
        int totalSpace = 0;
        for (auto ch : tasks) {
            if (lastPos[ch] != -1 && curPos - lastPos[ch] < k) {
                int spaces = k - (curPos - lastPos[ch]);
